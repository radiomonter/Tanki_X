namespace Tanks.Lobby.ClientGarage.Impl.Tutorial
{
    ...
    public class ModulesTutorStep8Handler : TutorialStepHandler
    {
        ...
        public override void RunStep(TutorialData data)
        {
            ...
            GameObject gameObject = this.modulesScreen.collectionView.turretCollectionView.activeTierCollectionList.transform.GetChild(0).gameObject;
            for (int i = 0; i < gameObject.transform.childCount; i++)
            {
                this.highlightedObjects.Add(gameObject.transform.GetChild(i).gameObject);
            }
            GameObject obj3 = this.modulesScreen.collectionView.turretCollectionView.passiveTierCollectionList.transform.GetChild(0).gameObject;
            for (int j = 0; j < obj3.transform.childCount; j++)
            {
                this.highlightedObjects.Add(gameObject.transform.GetChild(j).gameObject);
            }
            foreach (GameObject obj4 in this.highlightedObjects)
            {
                TutorialCanvas.Instance.AddAdditionalMaskRect(obj4);
            }
        ...
        {
            internal ModulesTutorStep8Handler $this;
